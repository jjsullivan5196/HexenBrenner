...
playerPack = struct.Struct('cfffc') #C struct object for handling network messages
            ...
            try:
                ...
                if clientOK:
                    ...
                    for plKey in players:
                        pl = players[plKey]
                        tagVals = (str(pl.id).encode(), pl.x, pl.y, pl.z, str(1).encode())
...
def serverMain():
    ...
    try:
        while True:
            ...
            tagVals = (str(newPlayer.id).encode(), newPlayer.x, newPlayer.y, newPlayer.z, str(1).encode())
            tagData = playerPack.pack(*tagVals)
